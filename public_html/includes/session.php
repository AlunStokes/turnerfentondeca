...
if (!isset($_SESSION['fist_name'])) {
	if ($_SESSION['member']) {
		...
		$ses_sql = mysqli_query($dbconfig, "SELECT * FROM applicants WHERE student_number = ".$_SESSION['student_number']." ");
		...
		$_SESSION['email'] = $row['email'];
		$_SESSION['student_number'] = $row['student_number'];
		$_SESSION['writtens'] = $row['writtens'];
		$_SESSION['can_bring_device'] = $row['can_bring_device'];
		$_SESSION['area'] = $row['area'];
		$_SESSION['unlocked_exam'] = $row['unlocked_exam'];
	}
}
